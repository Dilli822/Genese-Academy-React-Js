...
function onGetSavedNotes(notes) {
    let my_notes_html='';
    notes.forEach(function (val,index) {
        // we have just created a whole html dom with the help of js
        let main_node = document.getElementById('myNotes');
        if(val) {
            let wrapperDiv = document.createElement('div');
            let dateTag = document.createElement('p');
            dateTag.innerText=getUserReadableDate(val.date);
            ...
            deleteDiv.addEventListener('click', deleteNote(index));
